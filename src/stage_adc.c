...
#define R_ACC (92.4214) //accelometer [(1 m/s/s)/(54.1 mV) / 1(GAIN) =  92.4214 m/s/s / 5V]
#define R_EX (31.25)
...
void setup_adc_init(void)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		for (j = 0; j < NROFM; j++)
		{
			...
			sum1 -= ad1;
			sum2 -= ad2;
			sum3 -= ad3; // offset neg sum calc
		}
		ad_avg[4 * i] = sum0 / NROFM;
		ad_avg[4 * i + 1] = sum1 / NROFM; // channel avg calc
		ad_avg[4 * i + 2] = sum2 / NROFM;
		ad_avg[4 * i + 3] = sum3 / NROFM;
		sum0 = 0.0;
		sum1 = 0.0;
		sum2 = 0.0;
		sum3 = 0.0;
